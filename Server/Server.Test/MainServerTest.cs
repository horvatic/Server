...
namespace Server.Test
{
    public class MainServerTest
    {
        ...
        {
            var mockDataMangaer = new MockZSocket();
            var webMaker = new WebPageMaker();
            Assert.NotNull(new MainServer(mockDataMangaer, webMaker, null, new MockDirectoryProcessor(),
        ...
        public void Make_Web_Server_LIVE_SOCKET_Hello_World()
        {
            ...
            var dataManager = new ZSocket(new IPEndPoint((IPAddress.Loopback), 7415));
        ...
        public void Make_Sure_New_Main_Server_Is_Not_Null()
        {
            var mockDataMangaer = new MockZSocket();
            var webMaker = new WebPageMaker();
            Assert.NotNull(new MainServer(mockDataMangaer, webMaker, "", new MockDirectoryProcessor(),
                new MockFileProcessor()));
        }
        ...
        public void Make_Web_Server_LIVE_SOCKETS()
        {
            ...
            var dataManager = new ZSocket(new IPEndPoint((IPAddress.Loopback), 9157));
            var server = new MainServer(dataManager, webMaker, @"Home", mockRead, new MockFileProcessor());
            server.RunningProcess(dataManager);
        }
